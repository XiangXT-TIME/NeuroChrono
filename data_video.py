...
class SiglipVideoProcessor:
    def __init__(self, image_processor):
        self.image_processor = image_processor
        
    def __call__(self, videos, return_tensors="pt"):
        """
        处理视频数据，输入形状应为 (num_frames, height, width, channels)
        返回形状为 (channels, num_frames, target_height, target_width)
        """
        # 输入验证
        if len(videos.shape) != 4:
            raise ValueError("Input video must be 4-dimensional: (num_frames, height, width, channels)")
            
        processed_frames = []
        
        # 逐帧处理
        for frame in videos:
            # 转换numpy数组为PIL图像（假设输入是0-255范围的uint8）
            if isinstance(frame, np.ndarray):
                frame = self.image_processor.to_pil_image(frame)
            ...
            processed = self.image_processor(
                ...
                do_rescale=self.image_processor.do_rescale,
        ...
        return {"pixel_values": video_tensor.squeeze(0)}
...
class BrainDataset(Dataset):
    def __init__(self, data_dir, video_size, fps, max_num_frames, skip_frms_num=3,
        ...
        # Auto-detect supervision cache (v2 sharded format preferred, v1 per-clip fallback)
        if not self.sf_targets_dir:
            dataset_root = get_paths().get("dataset_root", "")
            # v2: sharded .pt format
            v2_candidate = os.path.join(dataset_root, "supervision_cache", "version_v1", "shards")
            # v1: per-clip .npy format (legacy)
            v1_candidate = os.path.join(dataset_root, "sf_targets")
            if os.path.isdir(v2_candidate):
                self.sf_targets_dir = v2_candidate
                self._sf_cache_format = "v2_sharded"
            elif os.path.isdir(v1_candidate):
                self.sf_targets_dir = v1_candidate
                self._sf_cache_format = "v1_per_clip"
            else:
    ...
    def get_item_func(self, idx):
        data_item = self.data_ann[idx]
        ...
        eeg = torch.cat(eeg_data_list, dim=0)

        text_list = text.split(".")
        if len(text_list) >= 2:
            start_index = random.randint(0, len(text_list) - 2)
            text = text_list[start_index]+ "." + text_list[start_index+1]+"."
        # text = text[:200]
        # caption = self.text_processor(text=text, padding="max_length", return_tensors="pt")
        caption = self.text_processor(
            text=text,
            padding="max_length",
            truncation=True,  # 限定长度为64
            max_length=64,
            return_tensors="pt"
        )
        video_path = data_item['video']
        vr = decord.VideoReader(video_path)
        frame_indices = np.arange(0,33,1)
        temp_frms = vr.get_batch(frame_indices)
        tensor_frms = torch.from_numpy(temp_frms) if type(temp_frms) is not torch.Tensor else temp_frms
        video = self.video_processor(tensor_frms.clone())
        tensor_frms = tensor_frms.permute(0, 3, 1, 2)  # [T, H, W, C] -> [T, C, H, W]
        tensor_frms = resize_for_rectangle_crop(tensor_frms, self.video_size, reshape_mode="center")
        tensor_frms = (tensor_frms - 127.5) / 127.5
