...
def save_checkpoint(iteration, model, optimizer,
    ...
    if hasattr(args, 'deepspeed') and args.deepspeed:
        if mpu.get_data_parallel_rank() == 0:
            ...
            print_rank0('Delete Condition and First stage model...')
...
def save_ds_checkpoint_no_optim(model, save_dir, tag=None, client_state={}, save_latest=True):
    ...
    # model_path = os.path.join("/inspire/hdd/project/roboticarm/fuyanwei-24014/gjx/CogVideo_fMRI/sat",save_dir,tag,"mp_rank_00_model_states.pt")
    # new_model = torch.load(model_path, map_location="cpu")
    # key_to_keep = "conditioner.embedders.0.transformer.encoder.embed_tokens.weight"
    # for key in list(new_model["module"].keys()):
    #     if "first_stage_model" in key and key != key_to_keep:
    #         del new_model["module"][key]
    # del new_model["frozen_param_fragments"]
