...
def broad_cast_batch(batch):
    ...
    fmri = [batch["fmri"], broadcast_shape]
    torch.distributed.broadcast_object_list(fmri, src=src, group=mpu.get_model_parallel_group())
    batch["fmri"] = fmri[0]

    mp4_shape = fmri[1][0]
    fps_shape = fmri[1][1]
    num_frames_shape = fmri[1][2]
...
def forward_step_eval(data_iterator, model, args, timers, only_log_video_latents=False, data_class=None):
    if mpu.get_model_parallel_rank() == 0:
        ...
        if len(batch_video["mp4"].shape) == 6:
            ...
            for i in range(b):
                for j in range(v):
                    fmri.append(batch_video["fmri"][j][i])
            batch_video["fmri"] = fmri
...
if __name__ == "__main__":
    ...
    os.environ["NCCL_TIMEOUT"] = "7200"        # 设置为 1 小时
    os.environ["NCCL_BLOCKING_WAIT"] = "1"     # 可选，避免 silent hang
    ...
    if not dist.is_initialized():
        dist.init_process_group(
            backend="nccl",
            init_method="env://",
            timeout=timedelta(seconds=7200)  # 2 小时
        )
    ...
    training_main(
        args,
        model_cls=SATVideoDiffusionEngineBrain,
